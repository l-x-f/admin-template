<template>
  <div class="tinymce-container editor-container" :class="{fullscreen:fullscreen}">
    ...
    <div class="editor-custom-btn-container">
      <editorImage class="editor-upload-btn" @success="imageSuccess"></editorImage>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async content2Url() {
      try {
        ...
        // 数据发送到后端
        let response = await axios.post(this.host, this.formData, {
          headers: { "Content-Type": "multipart/form-data" }
        });
        let data = response.request.responseURL + this.filename;
        // 返回一个promise
        return data;
      } catch (error) {
        console.log(error);
